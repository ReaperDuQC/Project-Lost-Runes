...
namespace NetcodePlus.Demo
{
    ...
    {
        public static bool DetectGround(Transform root, Vector3 center, float hdist)
        {
            LayerMask all = ~0;
            return DetectGround(root, center, hdist, hdist + 1f, all, out float _, out Vector3 _);
        }

        public static float GetGroundDistance(Transform root, Vector3 center, float max_dist)
        {
            LayerMask all = ~0;
            bool valid = DetectGround(root, center, max_dist, max_dist, all, out float dist, out Vector3 _);
            return valid ? dist : max_dist;
        }

        //Detect if object is grounded, the ground normal, and ground distance from root
        public static bool DetectGround(Transform root, Vector3 center, float hit_dist, float max_dist, LayerMask ground_layer, out float ground_distance, out Vector3 ground_normal)
        {
            RaycastHit hd;
            ground_normal = Vector3.up;
            ground_distance = max_dist;

            //Find ground distance, add extra longer raycast in case not enough are found (like on edge of slope)
            bool fd = Physics.Raycast(center, Vector3.down, out hd, max_dist, ground_layer.value, QueryTriggerInteraction.Ignore);
            if (fd)
            {
                ground_distance = (root.position - hd.point).y;
                ground_normal = FlipNormalUp(hd.normal);
            }

            //Debug.DrawRay(center, Vector3.down * hdist);
            return fd && ground_distance < hit_dist;
        }

        public static bool DetectGroundArea(Transform root, Vector3 center, float hdist, float radius)
        {
            LayerMask all = ~0;
            return DetectGroundArea(root, center, hdist, hdist + 1, radius, all, out float _, out Vector3 _);
        }

        public static bool DetectGroundArea(Transform root, Vector3 center, float hdist, float radius, LayerMask ground_layer, out float ground_distance, out Vector3 ground_normal)
        {
            return DetectGroundArea(root, center, hdist, hdist + 1, radius, ground_layer, out ground_distance, out ground_normal);
        }

        public static bool DetectGroundArea(Transform root, Vector3 center, float hit_dist, float max_dist, float radius, LayerMask ground_layer, out float ground_distance, out Vector3 ground_normal)
        {
            Vector3 p1 = center;
            ...
            bool f1 = Physics.Raycast(p1, Vector3.down, out h1, hit_dist, ground_layer.value, QueryTriggerInteraction.Ignore);
            bool f2 = Physics.Raycast(p2, Vector3.down, out h2, hit_dist, ground_layer.value, QueryTriggerInteraction.Ignore);
            bool f3 = Physics.Raycast(p3, Vector3.down, out h3, hit_dist, ground_layer.value, QueryTriggerInteraction.Ignore);
            bool f4 = Physics.Raycast(p4, Vector3.down, out h4, hit_dist, ground_layer.value, QueryTriggerInteraction.Ignore);
            bool f5 = Physics.Raycast(p5, Vector3.down, out h5, hit_dist, ground_layer.value, QueryTriggerInteraction.Ignore);
            ...
            //Find ground distance, add extra longer raycast in case not enough are found (like on edge of slope)
            bool fd = Physics.Raycast(p1, Vector3.down, out hd, max_dist, ground_layer.value, QueryTriggerInteraction.Ignore);
            if (is_grounded)
            {
                ...
                ground_distance = (root.position - hit_center).y;
            }
